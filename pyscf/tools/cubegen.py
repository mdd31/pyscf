...
def mep(mol, outfile, dm, nx=80, ny=80, nz=80, pad=4.0, gridspacing=None):
    ...
    Args:
        ...
        ny (int): Number of grid point divisions in y direction.
        nz (int): Number of grid point divisions in z direction.
        pad (float): Amount of padding (in Angstrom) in all dimensions
                     that will be applied in the automatic construction of the
                     rectangular grid volume based on the geometry of the system.
        gridspacing (float):  Distance, in Angstroms, between points in the grid,
                              in all dimensions. This will override the nx,ny,nz
                              the parameters. Note the following values:
               value/Angstroms  points/Bohr     Gaussian grid term
...
def mep_for_coords(mol, dm, coords):
    """This calculates the molecular electrostatic potential (MEP) value at the points in coords.

    Args:
        mol (Mole): Molecule to calculate the MEP for.
        outfile (str): Name of Cube file to be written.
        coords ((N,3) array): Array of the coordinates of points to calculate MEP at.
    Returns:
        mep_values ((N) array): Array of MEP values for points, using the same
                                ordering as coordinates.
    """
...
def isomep(mol, outfile, dm, electronic_iso=0.002, iso_tol=0.00003, nx=80,
           ny=80, nz=80, pad=4.0, gridspacing=None):
